...
useEffect(() => {
    ...
    if (slugLang !== language) {
      ...
      const newPath = language === 'EN' ? '/en/our-products' : '/urunlerimiz';
  ...
  const handleBack = () => {
    navigate(language === "EN" ? "/en/our-products" : "/urunlerimiz");
  };

  if (loading) return <LoadingSpinner text={t("loadingProduct")} />;
  if (!post) return <NotFound text={t("productNotFound")} link="/urunlerimiz" linkText={t("backToAllProducts")} />;
    ...
    <>
      ...
      <main className="pt-20 max-w-6xl mx-auto px-6 pb-16">
        <div className="bg-white rounded-2xl shadow-xl p-10 max-w-4xl mx-auto">
          ...
          <BackButton onClick={handleBack} text={t("backToAllProducts")} />
