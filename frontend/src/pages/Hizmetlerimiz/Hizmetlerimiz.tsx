...
function ServiceCard({ post, onClick }: { post: Blog; onClick: () => void }) {
      ...
      <div className="h-48 relative overflow-hidden">
        {post.image ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-amber-50">
            <span className="text-amber-600 font-semibold text-sm">TechSİN Service</span>
          </div>
        )}
      </div>
  ...
  return (
    <>
      <Helmet>
        <title>
          {language === "EN"
            ? "TechSİN Solutions | Our Services"
            : "TechSİN Solutions | Servislerimiz"}
        </title>
      </Helmet>
        ...
        <section className="relative bg-gradient-to-br from-blue-600 via-blue-700 to-purple-700 text-white py-28 px-6 text-center">
          <div className="max-w-4xl mx-auto relative z-10">
            <h1 className="text-5xl font-extrabold mb-4">
              {language === "EN" ? "Our Services" : "Servislerimiz"}
            </h1>
            <p className="text-lg md:text-xl text-blue-100 leading-relaxed">
              {language === "EN"
                ? "We provide professional IT solutions to streamline your business processes."
                : "İş süreçlerinizi kolaylaştıracak profesyonel bilişim çözümleri sunuyoruz."}
            </p>
          </div>
                ...
                <motion.div key={post._id} variants={itemVariants}>
                  <ServiceCard
                    ...
                    onClick={() =>
                      navigate(
                        language === "EN"
                          ? `/en/our-services/${post.slug}`
                          : `/hizmetlerimiz/${post.slug}`
                      )
                    }
                  />
                </motion.div>
